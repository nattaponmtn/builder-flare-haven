-- =====================================================
-- Multi-Company Support Implementation for CMMS (Corrected for TEXT IDs)
-- =====================================================
-- This script adds multi-company support to the work order and PM system
-- Note: This database uses TEXT type for IDs, not UUID
...
-- Make company_id NOT NULL after populating existing data (optional)
-- ALTER TABLE assets 
...
CREATE TABLE IF NOT EXISTS user_companies (
    id TEXT PRIMARY KEY DEFAULT substr(md5(random()::text), 1, 10),
    user_id UUID NOT NULL, -- This remains UUID as it references auth.users
...
ADD COLUMN IF NOT EXISTS parent_company_id TEXT REFERENCES companies(id);

-- 7. Create enhanced PM schedules table with multi-company support
-- =====================================================
CREATE TABLE IF NOT EXISTS pm_schedules (
    id TEXT PRIMARY KEY DEFAULT 'PMS-' || substr(md5(random()::text), 1, 10),
    company_id TEXT REFERENCES companies(id) NOT NULL,
    pm_template_id TEXT REFERENCES pm_templates(id) NOT NULL,
    ...
    auto_assign_to_user_id UUID, -- References user_profiles which uses UUID
    ...
    created_at TIMESTAMP DEFAULT NOW(),
    updated_at TIMESTAMP DEFAULT NOW(),
    created_by UUID, -- References user_profiles which uses UUID
...
CREATE TABLE IF NOT EXISTS pm_schedule_instances (
    id TEXT PRIMARY KEY DEFAULT 'PSI-' || substr(md5(random()::text), 1, 10),
    pm_schedule_id TEXT REFERENCES pm_schedules(id) NOT NULL,
    work_order_id TEXT REFERENCES work_orders(id) NOT NULL,
...
BEGIN
    ...
    v_wo_number := 'PM-' || TO_CHAR(NOW(), 'YYYYMMDD') || '-' || substr(md5(random()::text), 1, 6);
    v_work_order_id := 'WO-' || v_wo_number;
    ...
    SELECT 
        'WOT-' || v_work_order_id || '-' || row_number() OVER (ORDER BY step_number),
        v_work_order_id,
        task_description,
        is_critical,
        false
...
DROP TRIGGER IF EXISTS update_pm_schedules_updated_at ON pm_schedules;
...
-- 13. Enable Row Level Security (RLS) - Optional, uncomment if needed
-- =====================================================
...
-- 14. Create RLS policies for company-based access (Optional, uncomment if needed)
-- =====================================================
/*
-- Policy for assets
...
-- 16. Add company_id to user_profiles if missing
-- =====================================================
ALTER TABLE user_profiles
ADD COLUMN IF NOT EXISTS company_id TEXT REFERENCES companies(id);

-- Update user_profiles with a default company if needed
-- UPDATE user_profiles SET company_id = 'LAK' WHERE company_id IS NULL;
...
AND table_name IN ('assets', 'work_orders', 'equipment_types', 'user_profiles')
ORDER BY table_name;

-- 2. Check the new tables
SELECT 
    table_name
FROM information_schema.tables
WHERE table_schema = 'public'
AND table_name IN ('pm_schedules', 'pm_schedule_instances', 'user_companies')
ORDER BY table_name;

-- 3. Test the views
-- SELECT * FROM company_hierarchy LIMIT 5;
-- SELECT * FROM company_work_order_stats;
-- SELECT * FROM user_accessible_companies WHERE user_id = (SELECT id FROM auth.users LIMIT 1);