...
export function WorkOrderDetail() {
    ...
    const loadWorkOrderData = async () => {
      ...
      try {
        ...
        if (woResult.data) {
          ...
          // Load related data
          const [tasksResult, assetResult, locationResult, systemResult] = await Promise.allSettled([
            tasksService.getByField('work_order_id', id),
            wo.asset_id ? assetsService.getById(wo.asset_id) : Promise.resolve({ data: null }),
            wo.location_id ? locationsService.getById(wo.location_id) : Promise.resolve({ data: null }),
            wo.system_id ? systemsService.getById(wo.system_id) : Promise.resolve({ data: null }),
          ]);
  ...
  const handleTaskComplete = async (taskId: string, value: string | number | boolean) => {
    try {
      const tasksService = createTableService('work_order_tasks');
      ...
      if (task.expected_input_type === 'number' || task.expected_input_type === 'measurement') {
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50/30 flex items-center justify-center">
        <Card className="w-96">
          <CardContent className="p-6 text-center">
            ...
            <h3 className="text-lg font-semibold text-slate-800 mb-2">ไม่พบใบสั่งงาน</h3>
            <p className="text-slate-600 mb-4">ใบสั่งงานที่ระบุไม่มีอยู่ในระบบ</p>
                ...
                <CardHeader>
                  <div className="flex items-center justify-between">
                    <CardTitle className="flex items-center gap-2">
                      ...
                      รายการงาน ({taskCompletion.completed}/{taskCompletion.total})
                    </CardTitle>
                    <div className="text-right">
                      <div className="text-sm font-medium text-slate-900">
                        {taskCompletion.percentage}% เสร็จสิ้น
                      </div>
                      <Progress value={taskCompletion.percentage} className="w-32 h-2 mt-1" />
                    </div>
                  </div>
                </CardHeader>
                              ...
                              <div className="flex items-start gap-4">
                                <div className="flex items-center gap-2 mt-1">
                                  <div className={cn(
                                    ...
                                    {task.is_completed ? <CheckCircle className="h-4 w-4" /> : index + 1}
                                  </div>
                                </div>
                                ...
                                <div className="flex-1">
                                  ...
                                  {!task.is_completed ? (
                                    <div className="space-y-3">
                                      {task.expected_input_type === 'checkbox' ? (
                                      ...
                                      ) : task.expected_input_type === 'number' || task.expected_input_type === 'measurement' ? (
                                        <div className="space-y-2">
                                          <div className="flex gap-2">
                                            <Input
                                              ...
                                              onKeyDown={(e) => {
                                                if (e.key === 'Enter') {
                                                  handleTaskComplete(task.id, (e.target as HTMLInputElement).value);
                                                }
                                              }}
                                              step="0.01"
                                            />
                                            <Button 
                                              ...
                                              onClick={(e) => {
                                                ...
                                                if (input?.value) {
                                                  handleTaskComplete(task.id, input.value);
                                                }
                                              }}
                                            >
                                              บันทึก
                                          ...
                                          {task.standard_min_value !== undefined && task.standard_max_value !== undefined && (
                                            <p className="text-xs text-slate-500">
                                              ช่วงค่าปกติ: {task.standard_min_value} - {task.standard_max_value}
                                            </p>
                                          )}
                                        </div>
                                      ) : (
                                        <div className="space-y-2">
                                          ...
                                          {task.standard_text_expected && (
                                            <p className="text-xs text-slate-500">
                                              ค่าที่คาดหวัง: {task.standard_text_expected}
                                            </p>
                                          )}
                                        </div>
                                      )}
                                    ...
                                    <div className="space-y-2">
                                      <div className="bg-green-100 border border-green-200 rounded-lg p-3">
                                        <div className="flex items-center justify-between">
                                          <div>
                                            <p className="text-sm font-medium text-green-900">ผลการทำงาน:</p>
                                            <p className="text-sm text-green-800">
                                              {task.actual_value_text || task.actual_value_numeric || 'ทำงานเสร็จสิ้น'}
                                            </p>
                                          </div>
                                          <Button
                                            variant="ghost"
                                      ...
                                      {task.completed_at && (
                                        <p className="text-xs text-slate-500">
                                          เสร็จเมื่อ: {new Date(task.completed_at).toLocaleString('th-TH')}
                                        </p>
                                      )}
                                    </div>
                                  )}
                ...
                <CardContent className="space-y-4">
                  <div className="space-y-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm font-medium text-slate-700">ผู้รับผ���ดชอบ</p>
                        <p className="text-sm text-slate-600">{workOrder.assigned_to || 'ไม่ระบุ'}</p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm font-medium text-slate-700">ผู้ขอ</p>
                        <p className="text-sm text-slate-600">{workOrder.requested_by || 'ไม่ระบุ'}</p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm font-medium text-slate-700">วันที่สร้าง</p>
                    ...
                    {workOrder.scheduled_date && (
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-sm font-medium text-slate-700">วันที่กำหนด</p>
                          <p className="text-sm text-slate-600">
                            {new Date(workOrder.scheduled_date).toLocaleDateString('th-TH')}
                          </p>
                        </div>
                      </div>
                    )}
                    ...
                    {workOrder.completed_at && (
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-sm font-medium text-slate-700">วันที่เสร็จ</p>
                          <p className="text-sm text-slate-600">
                            {new Date(workOrder.completed_at).toLocaleDateString('th-TH')}
                          </p>
                        </div>
                      </div>
                    )}
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm font-medium text-slate-700">ชั่วโมงที่ประมาณ</p>
                        <p className="text-sm text-slate-600">{workOrder.estimated_hours || 0} ชม.</p>
                      </div>
                    </div>
                    ...
                    {workOrder.actual_hours && (
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-sm font-medium text-slate-700">ชั่วโมงจริง</p>
                          <p className="text-sm text-slate-600">{workOrder.actual_hours.toFixed(2)} ชม.</p>
                        </div>
                      </div>
                    )}
                    ...
                    {workOrder.total_cost && (
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-sm font-medium text-slate-700">ค่าใช้จ่ายรวม</p>
                          <p className="text-sm text-slate-600">฿{workOrder.total_cost.toLocaleString()}</p>
                        </div>
                      </div>
                    )}
                  </div>
                  ...
                  <CardContent className="space-y-3">
                    {asset && (
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-sm font-medium text-slate-700">อุปกรณ์</p>
                          <p className="text-sm text-slate-600">{asset.serial_number}</p>
                        </div>
                      </div>
                    )}
                    ...
                    {location && (
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-sm font-medium text-slate-700">สถานที่</p>
                    ...
                    {system && (
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-sm font-medium text-slate-700">ระบบ</p>
